...
<title>VOC Disease Detection</title>
...
.ai-panel{background:linear-gradient(145deg,rgba(20,20,45,.97),rgba(10,10,30,.99));border-radius:20px;padding:28px 32px;margin-bottom:28px;border:1px solid rgba(0,212,255,.25);box-shadow:0 0 40px rgba(0,100,255,.08)}
...
.advice-box{background:rgba(0,212,255,.06);border-left:3px solid #00d4ff;border-radius:8px;padding:14px 18px;font-size:.95rem;color:#ccc;line-height:1.6}
...
<div class="container">
  ...
  <div class="ai-panel">
    ...
    <div class="prediction-grid">
      ...
      <div class="pred-card"><div class="label">Disease / Condition</div><div class="value" style="font-size:1.1rem" id="pred-disease">&mdash;</div></div>
    ...
    <div class="advice-box"><strong>&#128161; Advice: </strong><span id="pred-advice">Click "Run AI Analysis" to get a health assessment.</span></div>
  </div>
  ...
  <div class="charts-grid">
    ...
    <div class="chart-container"><div class="chart-title">MQ3 &mdash; Alcohol</div><canvas id="mq3Chart"></canvas></div>
    <div class="chart-container"><div class="chart-title">MQ4 &mdash; Methane</div><canvas id="mq4Chart"></canvas></div>
    <div class="chart-container"><div class="chart-title">MQ135 &mdash; VOC</div><canvas id="mq135Chart"></canvas></div>
...
async function updateDashboard(){try{const res=await fetch("/latest/"+DEVICE_ID,{cache:"no-cache"});if(res.status===404){setStatus("Waiting for ESP32 data...","warning");return}if(!res.ok){setStatus("Backend error HTTP "+res.status,"error");return}const d=await res.json();const mq7=Number(d.MQ7??0),mq3=Number(d.MQ3??0),mq4=Number(d.MQ4??0),mq135=Number(d.MQ135??0),voc=Number(d.VOC??0);document.getElementById("mq7").textContent=mq7;document.getElementById("mq3").textContent=mq3;document.getElementById("mq4").textContent=mq4;document.getElementById("mq135").textContent=mq135;document.getElementById("voc").textContent=voc.toFixed(2);setStatus("LIVE "+new Date().toLocaleTimeString());const label=new Date().toLocaleTimeString();hist.labels.push(label);hist.mq7.push(mq7);hist.mq3.push(mq3);hist.mq4.push(mq4);hist.mq135.push(mq135);hist.voc.push(voc);if(hist.labels.length>MAX_POINTS){hist.labels.shift();ids.forEach(id=>hist[id].shift())}ids.forEach(id=>{charts[id].data.labels=hist.labels;charts[id].data.datasets[0].data=hist[id];charts[id].update("none")})}catch(e){setStatus("Cannot reach backend","error")}}
async function runPrediction(){const btn=document.getElementById("predictBtn");btn.disabled=true;btn.innerHTML='<span class="spinner"></span>Analyzing...';document.getElementById("pred-advice").textContent="Claude AI is analyzing...";try{const res=await fetch("/predict/"+DEVICE_ID,{cache:"no-cache"});if(!res.ok){document.getElementById("pred-advice").textContent="Failed HTTP "+res.status;return}const p=await res.json();const se=document.getElementById("pred-status");se.textContent=p.status;se.className="value status-"+p.status;document.getElementById("pred-disease").textContent=p.disease||"—";const re=document.getElementById("pred-risk");re.textContent=p.risk_level||"—";re.className="value risk-"+(p.risk_level||"Low");document.getElementById("pred-confidence").textContent=p.confidence||"—";document.getElementById("pred-advice").textContent=p.advice||"—";loadPredictionHistory()}catch(e){document.getElementById("pred-advice").textContent="Error: "+e.message}finally{btn.disabled=false;btn.innerHTML="&#9889; Run AI Analysis"}}
async function loadPredictionHistory(){try{const res=await fetch("/predictions/"+DEVICE_ID,{cache:"no-cache"});if(!res.ok)return;const preds=await res.json();if(!preds.length)return;document.getElementById("historyBody").innerHTML=preds.map(p=>"<tr><td>"+p.timestamp+"</td><td class='status-"+p.status+"'>"+p.status+"</td><td>"+p.disease+"</td><td>"+p.confidence+"</td><td style='font-size:.82rem'>"+p.advice+"</td></tr>").join("")}catch(e){}}
